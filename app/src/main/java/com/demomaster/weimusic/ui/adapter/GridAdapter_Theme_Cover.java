...
public class GridAdapter_Theme_Cover extends BaseAdapter {
    ...
    private ThemeConstants.CoverType type = ThemeConstants.CoverType.withSystem;

    //适配器初始化
    public GridAdapter_Theme_Cover(Context context, Object datas, ThemeConstants.CoverType type) {
        mContext = context;
        this.type = type;
        if (this.type == ThemeConstants.CoverType.customPicture) {
            this.datas = (List<String>) datas;
        } else if (this.type == ThemeConstants.CoverType.withSystem){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (type == ThemeConstants.CoverType.withSystem) {
            ...
            try {
                int val = QDSharedPreferences.getInstance().getInt(Constants.Key_Theme_Cover_System, 0);
                if (coverType == type && color == val) {
            ...
            try {
                String val = QDSharedPreferences.getInstance().getString(Constants.Key_Theme_Cover_Custom, null);
                if(coverType==type&&val!=null&&path.equals(val)){
                    viewHolder.iv_select.setVisibility(View.VISIBLE);
                }else {
                    viewHolder.iv_select.setVisibility(View.GONE);
                }
